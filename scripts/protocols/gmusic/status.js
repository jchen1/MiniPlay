var music_status = {
  ...
  get_thumb: function () {
    if (
      document
        .querySelector('paper-icon-button[data-rating="5"]')
        .getAttribute("title") === "Undo thumb-up"
    ) {
      return ThumbEnum.UP;
    } else if (
      document
        .querySelector('paper-icon-button[data-rating="1"]')
        .getAttribute("title") === "Undo thumb-down"
    ) {
  ...
  get_shuffle: function () {
    return document
      .querySelector('paper-icon-button[data-id="shuffle"]')
  ...
  get_repeat: function () {
    switch (
      document
        .querySelector('paper-icon-button[data-id="repeat"]')
        .getAttribute("title")
        .split(" ")[1]
    ) {
      case "Off.":
  ...
  get_playlist: function () {
    var playlist_root = document.querySelector(
      "#queueContainer > .queue-song-table > .song-table > tbody"
    );
    if (!playlist_root) {
      return [];
    }
    ...
    for (var i = 0; i < playlist_count; i++) {
      ...
      item.artist = playlist_item.querySelector(
        'td[data-col="artist"] > span > a'
      ).innerText;
      item.album = playlist_item.querySelector(
        'td[data-col="album"] > span > a'
      ).innerText;
      item.album_art = playlist_item
        .querySelector("span > img")
        .getAttribute("src");
      item.total_time = playlist_item.querySelector(
        'td[data-col="duration"] > span'
      ).innerText;
      item.total_time_s = this.get_time(item.total_time);

      item.play_count = playlist_item.querySelector(
        'td[data-col="play-count"] > span'
      );
      if (item.play_count == null) item.play_count = "0";

      item.currently_playing = playlist_item.classList.contains(
        "currently-playing"
      );
  ...
  update: function () {
    ...
    this.artist_id =
      this.artist_id == null ? "" : this.artist_id.getAttribute("data-id");
    ...
    this.status =
      document
        .querySelector('paper-icon-button[data-id="play-pause"]')
        .getAttribute("title") == "Pause"
        ? StatusEnum.PLAYING
        : StatusEnum.PAUSED;
    this.volume = parseInt(
      document.querySelector("#material-vslider").getAttribute("aria-valuenow")
    );
